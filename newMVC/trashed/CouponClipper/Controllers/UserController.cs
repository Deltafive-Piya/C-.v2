...
public class CouponController : Controller
{
    ...
    {
        List<Coupon> AllCoupons = _context.Coupons.Include(wed => wed.Organizer).Include(wed => wed.AllUsers).ToList();
    ...
    {
        Coupon? wed = _context.Coupons.Include(p => p.Organizer)
    ...
    [HttpPost("coupon/{id}/used")]
    public IActionResult AddUser(int id)
    {
        int UUID = (int)HttpContext.Session.GetInt32("UserId");
        CouponHasUsers used = _context.CouponHasUsers.FirstOrDefault(r => r.CouponId == id && r.UserId == UUID);
        if (used == null)
        {
            CouponHasUsers isUsedd = new()
            {
                UserId = UUID,
                CouponId = id
            };
            _context.Add(isUsedd);

        }
        else
        {
            _context.Remove(used);
        }
        _context.SaveChanges();
        return RedirectToAction("Dashboard");
    }
